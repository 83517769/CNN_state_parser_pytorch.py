...
class CNNModel(torch.nn.Module):
	...
	def evaluate(self):
		...
		for input_x, label in self.valid_dataloader:
			...
			#print("output", output)
			#print("label", label)
			validation_accs.append(torch.sum(output.cpu() == label.cpu()) / shape[0] / shape[1])
	...
	def decode_state(self, input_x):
		...
		output = output.data.cpu().numpy()[0] # TODO
...
def main():
	CLASSES = [14]
	#model = CNNModel(CLASSES,pretrained_model_pth=None)
	model = CNNModel(CLASSES, pretrained_model_pth=r'C:\Users\port\Desktop\DRL-AHLSP\model_weights\weights_20180505_1508/parser_epoch_17_loss_7.19790995944436e-05_valacc_0.9992972883597884.t7')
	#model.model_train()
